...
def add_gripper(root, side, cfg):
    ...
    palm = element(root, 'link', name=p+'gripper_base_link')
    ...
    for index, origin_sign in enumerate((-1, 1)):
        ...
        joint_name = p+'gripper_'+finger+'_finger_joint'
        ...
        if index == 1:
            element(joint, 'mimic', joint=p+'gripper_left_finger_joint', multiplier=-1)
    ...
    fixed(root, p+'palm_to_tcp', p+'gripper_base_link', p+'gripper_tcp', g['tcp_xyz'], g['tcp_rpy'])


def control_fragment(share, side, mode, arms, hardware=None):
    """Expand the standalone ros2_control Xacro fragment for one side."""
    if mode == 'gazebo':
        arm_plugin = gripper_plugin = 'gazebo_ros2_control/GazeboSystem'
    elif mode == 'mock':
        arm_plugin = gripper_plugin = 'mock_components/GenericSystem'
    else:
        arm_plugin = 'fairino_hardware/FairinoHardwareInterface'
        gripper_plugin = 'ros2_hkv_gripper/GripperHardwareInterface'
    values = {
        'prefix': side+'_', 'arm_plugin': arm_plugin,
        'gripper_plugin': gripper_plugin,
    }
    for i, value in enumerate(arms[side]['initial'], 1):
        values[f'initial_j{i}'] = str(value)
    if mode == 'real':
        values.update({
            'robot_ip': hardware[side]['robot_ip'],
            'serial_port': hardware[side]['serial_port'],
            **{key: str(value) for key, value in hardware['gripper'].items()},
            'gripper_closed_position': '0.1',
        })
    document = xacro.process_file(str(Path(share)/'urdf/my_robot.ros2_control.xacro'),
                                  mappings=values)
    fragment = ET.fromstring(document.toxml())
...
def build_model(share, scene_file, arms, mode, controller_file='', hardware=None):
    ...
    for side in SIDES:
        for control_node in control_fragment(share, side, mode, arms, hardware):
            root.append(control_node)
    if mode == 'gazebo':
...
def controllers(mode, side=None):
    ...
    for arm in SIDES if side is None else (side,):
        ...
        grip_kind = 'position_controllers/GripperActionController'
        ...
        result[names[2]] = {'ros__parameters': {
            'joint': arm+'_gripper_left_finger_joint', 'goal_tolerance': .002,
            'max_effort': 0.0, 'allow_stalling': True,
            'stall_velocity_threshold': .001, 'stall_timeout': 1.0,
...
def semantic(root, arms):
    ...
    for side in SIDES:
        ...
        element(group, 'joint', name=side+'_gripper_left_finger_joint')
        element(srdf, 'end_effector', name=side+'_hkv', parent_link=side+'_gripper_base_link',
        ...
        element(srdf, 'disable_collisions', link1=side+'_wrist3_link',
                link2=side+'_gripper_base_link', reason='Mounting')
...
def moveit_config(root, arms, share):
    ...
    for side in SIDES:
        ...
        for suffix, kind, action, joints in (
                ...
                ('gripper_controller', 'GripperCommand', 'command',
                 [side+'_gripper_left_finger_joint'])):
            name = side+'_'+suffix
