...
运行时应先看到 `Grasp-first preflight`、`Seeded Cartesian preflight complete`、`Grasp-first plan selected`，接近执行成功后出现 `Revalidating prepared descent`。这一抓取下降阶段不再出现重新请求 `Cartesian request` 的流程；之后抬升或扫描仍可能使用该服务。
