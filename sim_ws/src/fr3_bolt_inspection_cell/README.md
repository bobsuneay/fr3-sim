...
这是基于 `fr3_dual_bolt_cell` 新建的独立 ROS 2 功能包。它复用原包的双臂模型、HKV 夹爪底座、控制器、MoveIt 配置和桌子，在自己的 launch 中增加点云识别、窄指尖、三个相机和完整任务流程。原包的启动方式不受影响；构建时必须同时保留原包。
...
2. 默认右手张开，移动到螺丝上方 50 mm，以约 8 mm/s 的保守分段速度下降，闭合窄指尖，再抬高 50 mm。
...
原 HKV 夹指沿螺丝方向约 40 mm 宽，不能让两只手同时夹住 25 mm 螺丝。本包保留 HKV 主体，替换为**沿螺丝方向宽 4 mm 的简化窄指尖**，分别夹住距零件中心 −6 mm 和 +6 mm 的杆部。新指尖开口定义为两指内侧面的实际距离 `gap = q_left + q_right`；不要套用旧 CAD 的 2.4 mm 残余间隙。该几何是仿真方案，需要重新设计加工图、强度和夹持力后才能制造。
