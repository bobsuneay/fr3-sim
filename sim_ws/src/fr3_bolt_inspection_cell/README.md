...
4. 右手回到交接基准姿态，左手以垂直夹爪姿态从侧面靠近螺丝另一段，闭合并确认接管后，右手才松开、撤离、回到初始姿态。
...
展示中心保持 `[0.25, 0.0, 1.15]` m，交接中心改为 `[0.35, 0.0, 1.00]` m：展示结束后右手将零件向前移 10 cm、下移 15 cm，离开头部附近再让左手接近。位置指零件中心，双臂 TCP 由各自抓取变换计算；运行时仍由 MoveIt 检查可达性和碰撞。可在 `config/inspection.yaml` 的 `handover_center` 调整。

垂直交接会依次预检两个侧向接近方向及两种腕部翻转（共四种姿态，始终保持夹爪相互垂直）。每个候选先求最终夹取点的碰撞感知逆解，再反向验证直线接近段，最后规划从当前状态到预接近位置的连接；三部分全部通过才发送左手运动。执行失败不会自动换姿态继续移动。所有候选失败时保留夹持，日志输出逆解错误与诊断碰撞对象；关闭碰撞的逆解仅用于诊断，不用于执行。

UI 的“展示旋转速度”滑条可实时设定 **0.25×～2.00×**，默认 1.00×，倍率基于下述配置速度。停止拖动 250 ms 后发送设定，界面显示任务节点确认的倍率。新倍率在下一段展示旋转轨迹生成时生效，正在执行的轨迹保持原速度；下降夹取、交接、撤离速度不受影响。节点重启恢复 1.00×。对应命令话题是 `/inspection/scan_speed_scale`（`std_msgs/msg/Float64`），状态服务返回 `scan_speed_scale`。

交接采用链环式垂直姿态：接收夹爪绕零件轴线相对递交夹爪旋转 90°，从侧面接近；两手闭合方向互相垂直。保持原来的轴向抓取偏移，不再通过向杆部内侧移动来实现交接。闭合前核验实际 TCP 位置及姿态。先确认双指持续接触并切换持有者，递交手张开、撤离后，接收手移动 10 mm 验证零件跟随，再继续拍摄。

界面新增“跳到左手交接”（服务 `/inspection/skip_to_handover`）。右手进入展示阶段后可点击：当前运动完成后跳过剩余右手视角，进入左手交接，不会同时发出另一组运动。右手展示失败或停止后，只要本次任务保留交接上下文，也可点击恢复。恢复会等待机械臂静止、确认右手仍持有零件、更新碰撞场景，再规划到交接姿态。左手接管验证通过后右手才松开。进入左手接近阶段后，此快捷入口禁用；重启节点后不保留恢复上下文。手动跳过会记录在报告中，左手仍执行正常拍摄流程。

`config/inspection.yaml` 中展示速度为 `scan_speed: 0.030` m/s、`scan_joint_speed: 0.24` rad/s、`scan_joint_acceleration: 0.60` rad/s²，TCP/关节速度上限为原来的两倍。仅定点展示旋转使用这些关节参数，下降和交接沿用各自速度。实际耗时受轨迹及仿真速度影响，日志会显示展示轨迹的仿真时长。
...
当前待机姿态将双臂收在桌面作业区前方：右臂关节角为 `[2.0, -1.6, 1.6, -1.25, -0.93, 0.23] rad`，左臂按身体中线镜像生成，见 `config/arms.yaml`。TCP 约为 `(0.339, ±0.298, 0.963) m`；第五关节保持弯曲，减少从原来向两侧伸展的姿态转入作业区的需要。该配置用于启动及正常流程中的回位；失败重试采用局部退离。离线已检查关节限位、镜像关系、模型自碰撞和桌面碰撞；这些检查不能替代 Gazebo 中的完整路径规划验证。修改初始值后需要重启 launch。

首次下降高度由启动时读取的原始 `finger.stl` 顶点、URDF 安装变换及夹指开度计算。当前模型在竖直抓取时，桌面 `Z=0.720 m`、指尖间隙 `5 mm` 对应 TCP 高度约 `0.7227 m`。到位后再次使用实际关节反馈核对指尖最低点；不再通过抬高抓取目标来绕过逆解失败。关节闭合遇到零件时允许停在接触位置，开度本身不作为成功判据，但仍必须通过辅助夹持校验和试抬确认；Gazebo 的固定关节辅助夹持不等于真实摩擦力验证。
...
辅助夹持要求两指各自持续接触目标零件至少 100 ms（仿真时间），允许的接触间断不超过 30 ms，穿透深度不超过 1.5 mm。夹爪开度仅记录用于诊断，不根据螺丝半径设置成功门槛，也不限定零件轴线必须沿夹爪某一轴。仍检查实际关节反馈有效及两指联动同步。空夹、单指接触、瞬间擦碰或明显穿透不会建立固定关节；日志打印接触年龄和持续时间。固定关节辅助及跟随验证不等于真实摩擦/力反馈验证。
...
为避免原始 CAD 网格之间的微小接触让 Gazebo 夹爪数值振荡，两个活动手指在仿真中关闭 `selfCollide`，使用较低刚度、较高阻尼的接触参数，并将关节阻尼/摩擦提高到 `15.0/0.40`；这只影响仿真手指的数值稳定性，不改变视觉网格、MoveIt 碰撞网格或夹持服务的接触门限。每次夹爪动作结束后，任务节点会记录目标开口和两个实际关节位置。
...
等待控制器和 MoveIt 启动，在面板中点击 **开始完整任务**。首次夹取失败或夹住前停止后，等待后台运动退出，点击 **重新夹取**，不必重启仿真。手动和自动重试都确认双臂未持有零件，等待运动停止，只张开当前抓取夹爪，从实际 TCP 位置竖直抬高 20 mm，再重新识别三帧点云并抓取；不再让双臂回初始姿态。退离保持当前 XY 和姿态，由 `inspection.yaml` 的 `retry_lift_height: 0.02` 设置抬高距离。退离失败时停止并报错；成功夹取后继续检测和交接流程。若仍失败，可再次点击重试，每次生成独立结果目录，`report.json` 中的 `retry_of` 关联前次记录。
...
关闭面板只关闭窗口，后台任务仍在运行。需要停止时先点击停止按钮或调用停止服务。任务执行中或尚未停止时拒绝重复启动；一旦首次夹持已经确认，后续抬升、扫描、交接失败或任务完成后不会开放“重新夹取”，也不会自动松开悬空零件。夹持状态无法确认、退出运动未停止、局部退离规划失败时重试会中止并说明原因。如果零件已被碰出配置 ROI，需先调整场景或 ROI，重试不会用旧点云绕过定位。不要同时在 RViz 中发送执行命令或另开夹爪控制程序。
...
`scene.yaml` 管桌面、螺丝和头部相机，`arms.yaml` 管双臂初始关节角。更改螺丝位置时同步改 ROI；若想左手先抓，除了 `first_arm: left`，还应把桌上螺丝和 ROI 移到左侧并重新检查可达性。改变零件尺寸时，同步修改 `scene.yaml` 的 `bolts` 和 `inspection.yaml` 中对应的长度、半径参数；启动会检查两处一致。点云筛选自动跟随尺寸配置；C++ 接触验证不再依赖螺丝尺寸，仍需检查 ROI、张开宽度、抓取偏移和双手空间是否适合新尺寸。参数单位为米，`shaft_radius`、`head_radius` 填半径。
...
本包提供自己的 `libfr3_inspection_grasp.so`，无需第三方 link-attacher。它检查双指持续接触目标物和关节同步，再建立辅助固定关节；交接时在同一仿真更新中切换唯一持有者，不瞬移零件。
