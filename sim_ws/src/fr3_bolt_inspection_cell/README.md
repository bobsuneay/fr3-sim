...
2. 默认右手张开，先验证接近终点能完整下降（最多尝试 3 个接近规划），再移动到螺丝上方 50 mm，以约 8 mm/s 的保守分段速度下降，闭合原始 HKV 夹爪，再抬高 50 mm。
...
本包保留原始 HKV 夹爪的视觉外形，但采用与 `fr3_dual_bolt_cell` 一致的稳定简化碰撞代理：法兰、主体、滑轨和手指滑块使用盒体，只有左右两根手指末端保留 `finger.stl` 形状碰撞。这样既保留真实指尖夹持轮廓，也避免完整 CAD 网格在法兰安装处产生数值抖动。为适配原始夹爪，演示螺丝已放大为总长 35 mm、杆径 8 mm、头部直径 12 mm；实际两手夹持的可达性和夹持空间仍需在仿真中重新确认。
...
若日志在 `DESCEND` 显示 `Cartesian path incomplete`，表示 MoveIt 没有规划出完整下降段，任务此前不会发送任何下降轨迹。现在单目标笛卡尔段会尝试逐点逆解备用规划：步长不超过 1 mm，每次使用上个解作为种子，逆解超时 0.3 秒；在重采样前检查关节跳变，同时检查关节插值中的全机器人碰撞和 TCP 偏离。完整通过后才发送轨迹。接近前也进行这一预检，不可行的构型会重新规划。失败日志会指出采样位置、逆解错误码，或 `link <-> object` 碰撞对；不会执行部分路径。诊断阶段可能临时请求不带碰撞检查的逆解来查明碰撞对象，该解仅用于诊断，不用于执行。
